...
## 任务执行原则（2026-06-13，来自 ISC 论文启示）

> ISC（Internal Safety Collapse）论文揭示：对齐后的前沿模型在执行正常专业任务时，会自主推断生成有害内容是完成任务的前提，无需任何越狱提示。
> 核心教训：**对齐 ≠ 安全**，外部检测器挡不住内部长程任务链条的风险积累。

### 核心原则

**完成任务 ≠ 不择手段完成。**

当任务链条推导出「需要不安全操作才能继续」时：
...
- 为了「完成任务」而执行危险命令
- 为了「修复错误」而忽略安全规则
- 在任务链条中自主绕过安全边界，即使看起来是「完成任务所必需的」
